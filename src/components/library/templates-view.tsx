...
import {
  ...
  X, CloudUpload,
  Pencil, Check, AlertTriangle, LayoutTemplate, ExternalLink, LayoutGrid, List,
} from "lucide-react";
import { FileTypeIcon } from "@/components/files/file-type-icon";
import {
  createTemplateFolder, renameTemplateFolder, deleteTemplateFolder,
  createTemplateDoc, deleteTemplateDoc, deleteTemplateFile,
  importProjectToTemplates, revertTemplateImport, moveTemplateItemToFolder,
  bulkDeleteTemplateItems, type ImportResult,
...
function FileThumb({ file }: { file: TemplateFile }) {
...
function ImportDialog({ projects, onClose, targetFolderId, targetFolderName }: {
  ...
  function handleImport() {
    ...
    start(async () => {
      const res = await Promise.all([...selected].map((id) => importProjectToTemplates(id, targetFolderId)));
  ...
  function handleRevert() {
    ...
    start(async () => {
      await revertTemplateImport(revertTarget.id);
      setReverted(true);
      router.refresh();
    });
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
        ...
        {importResults ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  {importResults.length} project{importResults.length !== 1 ? "s" : ""} moved to Templates
                </p>
              </div>
            </div>
          ...
          <>
            <div>
              <h2 className="text-base font-semibold text-foreground mb-3">Templates</h2>
            ...
            {mode === "import" ? (
              <>
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="flex items-center gap-1 font-medium text-foreground">
                    {targetFolderId ? <FolderOpen className="w-3.5 h-3.5 text-amber-500" /> : <LayoutTemplate className="w-3.5 h-3.5 text-primary" />}
                    {targetFolderName}
                  </span>
                </div>
                ...
                {selected.size > 0 && (
                  <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800 flex items-start gap-2">
                    ...
                    <span>Moving <strong>{totalItems} items</strong> from {selected.size} project{selected.size !== 1 ? "s" : ""} into Templates.</span>
                  </div>
                )}
              ...
              <>
                <p className="text-sm text-muted-foreground">Move all current Templates content back to a project. Useful if you imported by mistake.</p>
                <div className="max-h-56 overflow-y-auto space-y-1 rounded-xl border border-border p-2">
                  {projects.map((p) => (
                ...
                {revertTarget && (
                  <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-800 flex items-start gap-2">
                    ...
                    <span>All Templates docs and files will move back to <strong>{revertTarget.name}</strong>. Template folders will be deleted.</span>
                  </div>
                )}
                <div className="flex gap-3">
                  <Button variant="outline" onClick={onClose} className="flex-1">Cancel</Button>
...
export function TemplatesView({
  ...
  const { startUpload, isUploading } = useUploadThing("templatesFileUploader", {
  ...
  function handleBulkDelete() {
    start(async () => {
      await bulkDeleteTemplateItems([...selectedDocs], [...selectedFiles]);
      clearSelection();
      router.refresh();
    });
  }
  ...
  function handleFolderDrop(e: React.DragEvent, folderId: string) {
    ...
    try {
      ...
      start(async () => {
        await moveTemplateItemToFolder(type, id, folderId);
        router.refresh();
      });
    } catch {}
  }
  ...
  function handleCreateFolder() {
    ...
    start(async () => {
      await createTemplateFolder(folderName.trim(), currentFolderId);
      setFolderName("");
  ...
  function handleRenameFolder(folderId: string) {
    ...
    start(async () => {
      await renameTemplateFolder(folderId, renameValue.trim());
      setRenamingId(null);
      router.refresh();
    });
  }
  ...
  function handleCreateDoc() {
    start(async () => {
      const doc = await createTemplateDoc("Untitled", currentFolderId);
      router.push(`/templates/docs/${doc.id}`);
    });
  }
  function handleDeleteDoc(docId: string) {
    start(async () => {
      await deleteTemplateDoc(docId);
      router.refresh();
    });
  }
  ...
  return (
    ...
    >
        ...
        <div>
          {breadcrumb.length > 0 && (
            <div className="flex items-center gap-1.5 text-sm text-muted-foreground mb-1">
              <Link href="/templates" className="hover:text-foreground transition-colors flex items-center gap-1">
                <LayoutTemplate className="w-3.5 h-3.5" /> Templates
              </Link>
              {breadcrumb.slice(0, -1).map((crumb) => (
                <span key={crumb.id} className="flex items-center gap-1.5">
                  <ChevronRight className="w-3 h-3" />
                  <Link href={`/templates?folder=${crumb.id}`} className="hover:text-foreground transition-colors">
                    {crumb.name}
                  </Link>
                </span>
              ))}
          ...
          <h1 className="text-xl font-semibold text-foreground">
            {breadcrumb.length > 0 ? breadcrumb[breadcrumb.length - 1]!.name : "Templates"}
          </h1>
        </div>
      ...
      {/* Page note — only shown at root level (templates is admin-only so always editable) */}
      {!currentFolderId && (
        <div className="mb-5">
          {noteEditing ? (
            <div className="flex items-start gap-2 p-3 rounded-xl border border-primary/40 bg-primary/5">
              <textarea
                ...
                placeholder="Write a brief introduction for these templates…"
                rows={3}
            ...
            <div
              className="group relative px-4 py-3 rounded-xl border border-border/60 bg-muted/30 text-sm text-muted-foreground leading-relaxed cursor-pointer hover:border-primary/30 hover:bg-primary/5 hover:text-foreground transition-colors"
          ...
          <div className={cardGrid}>
            {docs.map((doc) => {
              ...
              return (
                ...
                >
                  <Link href={`/templates/docs/${doc.id}`}>
                    <div className={cn(
          ...
          {fileView === "grid" ? (
            <div className={cardGrid}>
              {files.map((file) => {
                ...
                return (
                  <div key={file.id} className="group relative" draggable onDragStart={(e) => handleDragStart(e, "file", file.id)}>
            ...
            <div className="rounded-xl border border-border/60 bg-card overflow-hidden divide-y divide-border/40">
              {files.map((file) => {
                ...
                return (
                  <div key={file.id} className={cn("group flex items-center gap-3 px-4 py-2.5 transition-colors", isSelected ? "bg-primary/5" : "hover:bg-muted/40")} draggable onDragStart={(e) => handleDragStart(e, "file", file.id)}>
                    ...
                    {ext && <span className="text-[10px] font-semibold px-2 py-0.5 rounded shrink-0 bg-muted text-muted-foreground">{ext}</span>}
      ...
      {importOpen && (
        <ImportDialog
          ...
          targetFolderName={breadcrumb.length > 0 ? breadcrumb[breadcrumb.length - 1]!.name : "Templates"}
        />
      )}
    </div>
  );
