...
import { Megaphone, Plus, Trash2, Loader2, X, AlertTriangle } from "lucide-react";
            ...
            <div key={a.id} className="rounded-2xl border border-border bg-card p-5 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                  {a.author.avatarUrl ? (
                    <img src={a.author.avatarUrl} alt={a.author.name} className="w-9 h-9 rounded-full object-cover shrink-0" />
                  ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {a.author.name}
                      {a.author.jobTitle && <span className="text-muted-foreground/60"> · {a.author.jobTitle}</span>}
              ...
              <p className="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap pl-12">{a.body}</p>
            </div>
          ))}
        </div>
      )}
